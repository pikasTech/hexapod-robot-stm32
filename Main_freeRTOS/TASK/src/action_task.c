...
/*声明部分LRL 和RLR腿高分别设置*/
void LRL_high_single(int time,
                     float h1,
                     float h2,
                     float h3);  // LRL顺序为LF RM LB
void RLR_high_single(int time,
                     float h1,
                     float h2,
                     float h3);  // RLR顺序为RF LM RB
...
void generate_Initial_And_End_Position(void) {
    if (STA_START == 1)
        Rf_use = 0, Rz_use = 0;  //启动时前伸比和前转比置1
    else
        Rf_use = Rf, Rz_use = Rz;
...
void startUp_Or_End(void) {
    ...
    if (STA_STAND == 1)  //静立时检测是否需要启动
    {
        ...
        if ((Order_use[0] > -0.2f && Order_use[0] < 0.2f) &&
            ...
            STA_STAND = 0, STA_START = 1;  //需要移动时启动
    }
...
void wether_Change_State(void) {
    ...
    if (LRL_phase >= 0 && LRL_phase < LRL_phase10)
        LRL_S = S10;
    if (LRL_phase >= LRL_phase10 && LRL_phase < LRL_phase0)
        LRL_S = S0;
    ...
    if (RLR_phase >= 0 && RLR_phase < LRL_phase10)
        RLR_S = S10;
    if (RLR_phase >= LRL_phase10 && RLR_phase < LRL_phase0)
        RLR_S = S0;
...
void action() {
    ...
    if (STA_STAND == 0) {
        ...
        if (LRL_S == S01) {
            ...
            h = ZhiXianGuiJi(LRL_phase0, LRL_phase0 + LRL_phase10 * (1 - R0f),
                             h_up, 0, LRL_phase);
        ...
        //摆动相0
            ...
            x = ZhiXianGuiJi(LRL_phase10, LRL_phase10 + 0.5f - 2 * LRL_phase10,
                             LRL_P2[0], LRL_P1[0], LRL_phase);
            y = ZhiXianGuiJi(LRL_phase10, LRL_phase10 + 0.5f - 2 * LRL_phase10,
                             LRL_P2[1], LRL_P1[1], LRL_phase);
            theat =
                ZhiXianGuiJi(LRL_phase10, LRL_phase10 + 0.5f - 2 * LRL_phase10,
                             LRL_theat2, LRL_theat1, LRL_phase);
        ...
        //过度相10
            ...
            if (LRL_phase < LRL_phase10 * R0f)
                LF_h = 0;
            else
                LF_h = ZhiXianGuiJi(LRL_phase10 * R0f,
                                    LRL_phase10 * R0f + LRL_phase10 * (1 - R0f),
                                    LF_h0, LF_hEnd, LRL_phase);
            ...
            if (LRL_phase < LRL_phase10 * R0f)
                ...
                h = ZhiXianGuiJi(LRL_phase10 * R0f,
                                 LRL_phase10 * R0f + LRL_phase10 * (1 - R0f), 0,
                                 h_up, LRL_phase);
            LRL_high(0, h);  //抬腿
#endif
        }
        //支撑相1
            ...
            x = ZhiXianGuiJi(0.5f, 0.5f + 0.5f, LRL_P1[0], LRL_P2[0],
                             LRL_phase);
            y = ZhiXianGuiJi(0.5f, 0.5f + 0.5f, LRL_P1[1], LRL_P2[1],
                             LRL_phase);
            theat = ZhiXianGuiJi(0.5f, 0.5f + 0.5f, LRL_theat1, LRL_theat2,
                                 LRL_phase);
        ...
        //过度相10
            ...
            if (RLR_phase < LRL_phase10 * R0f)
                RF_h = 0;
            else
                RF_h = RF_h0 + RF_kh * (RLR_phase - LRL_phase10 * R0f);
            ...
            if (RLR_phase < LRL_phase10 * R0f)
                ...
                LM_h = LM_h0 + LM_kh * (RLR_phase - LRL_phase10 * R0f);
            ...
            if (RLR_phase < LRL_phase10 * R0f)
                ...
                RB_h = RB_h0 + RB_kh * (RLR_phase - LRL_phase10 * R0f);
            ...
            if (RLR_phase < LRL_phase10 * R0f)
                ...
                h = 0 + kh * (RLR_phase - LRL_phase10 * R0f);
            RLR_high(0, h);  //抬腿
...
void action_task(void* pvParameters) {
    while (1) {
        if (flag_action_run_single == 1 ||
            flag_action_run_continiue == 1)  //出现（单次/连续）运行触发信号
        {
            ...
            } else if (STA_ACTION == TEST)  //测试模式
            {
                ...
                {
                    for (ChaBu_n_TEST = 0; ChaBu_n_TEST < ChaBu_NUM_TEST;
                         ChaBu_n_TEST++) {
                        ChaBu(time_TEST_TaiJiao, ChaBu_NUM_TEST, 0, h_TEST,
                              ChaBu_n_TEST, &ChaBu_dt, &ChaBu_hNow);
                        LRL_high(ChaBu_dt, ChaBu_hNow);
                        flag_action_run_single = 2;
                        vTaskDelay(ChaBu_dt);
                        //                      while(flag_action_run_single!=0)vTaskDelay(10);//等待运行结束
                    }
                    while (STA_TEST_TaiJiao == 1)
                        vTaskDelay(10);
                    ;                              //停止
                } else if (STA_TEST_TaiJiao == 0)  //落脚
                {
                    ...
                    for (ChaBu_n_TEST = 0; ChaBu_n_TEST < ChaBu_NUM_TEST;
                         ChaBu_n_TEST++) {
                        ChaBu(time_TEST_LuoJiao, ChaBu_NUM_TEST, h_TEST, 0,
                              ChaBu_n_TEST, &ChaBu_dt, &ChaBu_hNow);
