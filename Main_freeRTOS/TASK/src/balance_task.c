...
//旋转后的上平面六点向量
double T03_RFx[3][1],T03_RMx[3][1],T03_RBx[3][1],T03_LFx[3][1],T03_LMx[3][1],T03_LBx[3][1];
double T03_RFxy[3][1],T03_RMxy[3][1],T03_RBxy[3][1],T03_LFxy[3][1],T03_LMxy[3][1],T03_LBxy[3][1];
...
//				ML3=Link([0       0           d3          -pi/2      0     ],'modified');
//				ML4=Link([0       0           d4          pi/2       0     ],'modified');
//				ML5=Link([0       0           d5          0          0     ],'modified');
//				ML6=Link([0       0           d6          0          0     ],'modified');
				...
				//计算 x26 y26 z26
				x26 =	\
				(z06*sin(tt2)*cos(tt1)*cos(tt1) + x06*cos(tt2)*cos(tt1) + z06*sin(tt2)*sin(tt1)*sin(tt1)	\
				+ y06*cos(tt2)*sin(tt1))/((cos(tt1)*cos(tt1) + sin(tt1)*sin(tt1))*(cos(tt2)*cos(tt2)			\
				+ sin(tt2)*sin(tt2)));
				
				y26 =	\
				-(x06*cos(tt1)*sin(tt2) + y06*sin(tt1)*sin(tt2) - z06*cos(tt1)*cos(tt1)*cos(tt2)					\
				- z06*cos(tt2)*sin(tt1)*sin(tt1))/((cos(tt1)*cos(tt1) + sin(tt1)*sin(tt1))*(cos(tt2)*cos(tt2)	\
				+ sin(tt2)*sin(tt2)));
				
				z26 =-(y06*cos(tt1) - x06*sin(tt1))/(cos(tt1)*cos(tt1) + sin(tt1)*sin(tt1));
				//计算 T3
				...
				(d5*g + d6*g*cos(tt5) + d6*y26*sin(tt5))/(d6*d6*cos(tt5)*cos(tt5) \
				+ d6*d6*sin(tt5)*sin(tt5) + d5*d5 + 2.0f*d5*d6*cos(tt5));
				ss4 =	\

				(d5*y26 - d6*g*sin(tt5) + d6*y26*cos(tt5))/(d6*d6*cos(tt5)*cos(tt5) \
				+ d6*d6*sin(tt5)*sin(tt5) + d5*d5 + 2.0f*d5*d6*cos(tt5));
				tt4=atan2(ss4,cc4);
...
{  
   ...
   float Bias;
	 ...
	 Pitch_Integral+=Bias;
	 balance=Kp_Pitch*Bias+Gyro*Kd_Pitch+Pitch_Integral*Ki_Pitch;   //===计算平衡控制的电机PWM  PD控制   kp是P系数 kd是D系数 
	 return balance;
}
float Roll_goal;
float Roll_Integral;
...
{  
   float Bias;
	 ...
	 Roll_Integral+=Bias;
   balance=Kp_Roll*Bias+Gyro*Kd_Roll+Roll_Integral*Ki_Roll;   //===计算平衡控制的电机PWM  PD控制   kp是P系数 kd是D系数 
	 return balance;
}
...
void		Balance_Pid_Contral(void)
{
		...
		if(theatx>20)theatx=20;
		if(theatx<-20)theatx=-20;
		if(theaty>20)theaty=20;
		if(theaty<-20)theaty=-20;
}
